...
export default function Header({ tabAktif, onGantiTab, kuota, onKeluar, tema, onGantiTema }) {
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Analisa kode &amp; latihan soal pemrograman
            </p>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <nav className="flex flex-1 gap-1 rounded-xl bg-slate-100 p-1 dark:bg-slate-800">
            {TABS.map((tab) => {
          ...
          <button
            ...
            className="shrink-0 rounded-lg px-2 py-2 text-xs font-medium text-slate-400 transition hover:text-slate-700 dark:hover:text-slate-200"
          >
            Keluar
          </button>
        </div>
...
function SisaKuota({ kuota }) {
  ...
  return (
    ...
    >
      Sisa {kuota.sisa}
      <span className="text-slate-400 dark:text-slate-500">/{kuota.batas}</span>
    </div>
  )
}
