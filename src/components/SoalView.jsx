...
export default function SoalView({ soal, dibuatAI = false, onKembali, onKerjakan }) {
  ...
  return (
    <article className="flex flex-col gap-4">
      ...
      <h2 className="text-xl font-semibold text-slate-900">{soal.judul}</h2>

      {dibuatAI && (
        <p className="rounded-lg bg-amber-50 px-4 py-2.5 text-xs leading-relaxed text-amber-800">
      ...
      <p className="text-sm leading-relaxed text-slate-600">{soal.cerita}</p>

      <div className="rounded-xl border-l-4 border-indigo-500 bg-indigo-50/50 px-4 py-3">
        <p className="text-[11px] font-semibold text-indigo-700">TUGASMU</p>
        <p className="mt-1 text-sm leading-relaxed text-slate-800">{soal.tugas}</p>
      </div>
      ...
      {soal.batasan?.length > 0 && (
        <section className="rounded-xl border border-slate-200 bg-white p-4">
          <h3 className="mb-2 text-sm font-semibold text-slate-900">Batasan</h3>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4">
        <h3 className="mb-2.5 text-sm font-semibold text-slate-900">Contoh</h3>
        <div className="flex flex-col gap-4">
          {soal.contoh?.map((c, i) => (
            <div key={i}>
              <p className="mb-1.5 text-[11px] font-medium text-slate-500">Contoh {i + 1}</p>
              <div className="grid gap-2 sm:grid-cols-2">
                <BlokTeks label="Masukan" isi={c.input} />
                <BlokTeks label="Keluaran" isi={c.output} />
              </div>
              {c.penjelasan && (
                <p className="mt-2 text-xs leading-relaxed text-slate-600">{c.penjelasan}</p>
              )}
            </div>
          ))}
        </div>
      ...
      {soal.petunjuk?.length > 0 && (
        <section className="rounded-xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-semibold text-slate-900">Petunjuk</h3>
          ...
          <div className="flex flex-col gap-2">
            {soal.petunjuk.slice(0, petunjukTerbuka).map((p, i) => (
              <div key={i} className="rounded-lg bg-slate-50 px-3 py-2.5">
                <p className="text-[11px] font-semibold text-slate-400">Petunjuk {i + 1}</p>
                <p className="mt-1 text-sm leading-relaxed text-slate-700">{p}</p>
              </div>
            ))}
          </div>
          ...
          {petunjukTerbuka < soal.petunjuk.length && (
            <button
              ...
              className="mt-2.5 rounded-lg border border-dashed border-slate-300 px-3 py-2 text-xs font-medium text-slate-600 transition hover:border-indigo-300 hover:text-indigo-700"
            >
...
function Kartu({ judul, children }) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-4">
      <h3 className="mb-1.5 text-sm font-semibold text-slate-900">{judul}</h3>
      <p className="text-xs leading-relaxed whitespace-pre-line text-slate-600">{children}</p>
    </section>
  )
}
