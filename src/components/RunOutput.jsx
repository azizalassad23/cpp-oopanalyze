const TAMPILAN = {
  berhasil: {
    ...
    kelas: 'bg-emerald-50 text-emerald-800 ring-emerald-100',
  },
  'gagal-compile': {
    ikon: '🚫',
    judul: 'Kode gagal dicompile',
    kelas: 'bg-rose-50 text-rose-800 ring-rose-100',
    catatan:
  ...
  'gagal-jalan': {
    ...
    kelas: 'bg-amber-50 text-amber-800 ring-amber-100',
    catatan:
  ...
  return (
    <section className="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-sm font-semibold text-slate-900">Hasil menjalankan kode</h3>
        <span className="text-[11px] text-slate-400">{hasil.detik.toFixed(1)} detik</span>
      </div>
...
function Kotak({ judul, isi, kosong, nada = 'biasa' }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <span className="text-[11px] font-medium text-slate-500">{judul}</span>
      <pre
